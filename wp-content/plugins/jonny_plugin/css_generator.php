...
add_action('wp_head','jonny_css_generator');
function jonny_css_generator()
{
	...
	$con =jonny_css_generator_custumize();
	/*******************************************************************/
	$F = $wp_filesystem->put_contents($jonny_filename, jonny_minify_css($con));


}

function jonny_css_generator_custumize()
{
	if (!current_user_can("administrator") || isset($wp_customize)) {
		return;
		exit();
	...
	return jonny_minify_css($con);


}


function jonny_replace_callback($matches) {
	return 'calc(' . preg_replace('#\s+#', "\x1A", $matches[1]) . ')';
}

function jonny_minify_css($input) {
	if(trim($input) === "") return $input;
	// Force white-space(s) in `calc()`
	if(strpos($input, 'calc(') !== false) {
		$input = preg_replace_callback('#(?<=[\s:])calc\(\s*(.*?)\s*\)#',jonny_replace_callback(), $input);
	}
	return preg_replace(
		array(
			// Remove comment(s)
